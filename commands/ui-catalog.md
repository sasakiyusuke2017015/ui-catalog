...
6. **クロスプロジェクト統合・分割判断**
   ...
   - **統合判断:**
     - 同じコンポーネントへの類似拡張 → 1つのより汎用的な実装に統合
     - 例: project/A で `Button` に `loading` 追加、project/B で `Button` に `spinner` 追加
       → `loading` prop + `loadingIndicator` slot として統合
     - 複数プロジェクトで似たコンポーネントが新規作成 → 共通コンポーネントとして統合
   - **分割判断:**
     - main のコンポーネントが肥大化（props 15個以上、300行以上）
     - 特定プロジェクトでしか使わない機能が混在 → コア機能と拡張機能に分離
     - 例: `DataTable` が複雑化 → `Table` (base) + `SortableTable` + `FilterableTable`
   - **判断基準の提示:**
     ```
     ┌─────────────────────────────────────────────────┐
     │ 統合・分割の検討                                  │
     ...
     ├─────────────────────────────────────────────────┤
     │ 選択: [1] 統合実行 [2] 分割実行 [3] スキップ      │
     └─────────────────────────────────────────────────┘
     ```
   - ユーザーの選択に基づいて実行
7. `VERSION_REGISTRY` の整合性を確認
8. 変更サマリーを報告（統合・分割の結果も含む）
9. 元のブランチに戻る: `git checkout <元のブランチ>`

---

### `distribute` — 全リモート同期 + project/* 配布

全リモートの main を双方向同期し、project/* ブランチに配布する。

**リモート構成:**
- `origin`: プライマリリモート — 全ブランチ管理（main + project/*）
- その他: セカンダリリモート — main のみ同期

リモートは動的に検出。`git remote -v` で登録された全リモートが対象。
`origin` 以外のリモートには main ブランチのみ同期（project/* は origin 専用）。
...
2. **各リモートの差分を確認**:
   ```bash
   for remote in $(git remote); do
     git log --oneline HEAD..$remote/main  # 未取り込み
...
│ リモート  │ 未取り込み  │ 未配布     │ 同期対象     │
├──────────┼────────────┼────────────┼─────────────┤
│ origin   │ 0 commits  │ 2 commits  │ 全ブランチ   │
│ github   │ 3 commits  │ 2 commits  │ main のみ   │
│ gitlab   │ 0 commits  │ 5 commits  │ main のみ   │
└──────────┴────────────┴────────────┴─────────────┘
