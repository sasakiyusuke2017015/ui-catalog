...
| ブランチ | 利用可能コマンド |
|---------|----------------|
| `project/*` | `absorb`, `apply`, `refine`, `status` |
| `main` | `merge`, `distribute`, `atomize`, `status` |
...
### `atomize` — 外部依存・プリミティブHTMLを ui-catalog 内部コンポーネントに分解

organisms/templates が以下を直接使用している箇所を検出し、ui-catalog 内の atoms/molecules への置き換えを提案・実行する:
- **外部ライブラリ**（Mantine、Radix 等）の直接インポート
- **プリミティブHTML**（`<div>`, `<button>`, `<select>` 等）の直接使用
