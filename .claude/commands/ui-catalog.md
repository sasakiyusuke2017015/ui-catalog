...
5. **PR レビュー・マージ**:
   ...
   - **`project/*` ブランチは削除しない**（→「ブランチライフサイクル」参照）

6. **ローカル main を最新化**:
   ```bash
   git pull origin main
   ```
   - マージ後、リモートの変更をローカル main に反映する
   - 後続の `ds`（配布）や `at`（atomize）がローカル main を基準に動作するため必須
...
8. **変更サマリーと配布**
   ...
   💡 次のステップ: /ui-catalog ds で全 project/* に配布
   ```
...
`project/*` ブランチと短命ブランチ（`feature/*`, `fix/*` 等）はライフサイクルが異なる。

| ブランチ種別 | 寿命 | マージ後の削除 | 用途 |
|------------|------|--------------|------|
| `project/*` | **長期** | **削除しない** | 各アプリ専用の開発ブランチ。`ds` で配布、`ab`/`ap` で継続開発 |
| `feature/*`, `fix/*` 等 | 短期 | 削除する | 単発の機能追加・バグ修正 |
| `sync/from-*` | 一時的 | マージ後に削除 | リモート間同期用の一時ブランチ |

**`mg` / `pr` での注意:**
- Squash Merge 時に `--delete-branch` を使うのは短命ブランチのみ
- `project/*` は `--delete-branch` を**付けない**
