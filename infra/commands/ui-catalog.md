...
| `mg` | merge | PR をマージ |
| `ds` | distribute | main を配布 |
| `at` | atomize | 依存最適化 |
| `rf` | refine | クリーンアップ |
| `st` | status | 状態確認 |
| `sv` | sync-versions | VERSION_REGISTRY ↔ versions.json 同期 |
...
| ソース | 経路 | 説明 |
|--------|------|------|
| project/* | `pr` → 各リモート PR | project ブランチから main への PR |
| 他リモート | `sync/from-<remote>` → 各リモート PR | 外部変更を各リモートに PR |

**リモート同士は同格なので、すべて PR 経由で取り込む。**
ローカル main を中心に、全リモートへ PR を作成する。
...
│ github/main                 │ 1     │ a1b2c3d feat: 外部コントリビュート     │
└─────────────────────────────┴───────┴─────────────────────────────────────┘
...
### `ds` (distribute) — main を全ターゲットに配布

ローカル main の変更を project/* ブランチ + 全リモートに配布する。
取り込みは行わない（取り込みは `merge` で行う）。

**配布方式:**
| ターゲット | 方式 | 理由 |
|-----------|------|------|
| project/* | cherry-pick | 同一リポジトリ内、ローカル操作のため安全 |
| 全リモート | PR 方式 | **リモート同士は同格 → 直 push 禁止** |
...
│ リモート  │ 差分       │ 状態                        │
├──────────┼────────────┼─────────────────────────────┤
│ origin   │ 2 commits  │ 📦 PR 作成可能               │
│ github   │ 2 commits  │ 📦 PR 作成可能               │
│ gitlab   │ 0 commits  │ ✅ 同期済み                  │
└──────────┴────────────┴─────────────────────────────┘
```

3. **project/* ブランチに cherry-pick**:
   ```bash
...
patterns/layouts の依存関係を最適化し、コンポーネントの重複を解消する:
- **外部ライブラリ**（Mantine、Radix 等）の直接インポート → primitives/components に置換
- **プリミティブHTML**（`<div>`, `<button>`, `<select>` 等）の直接使用 → primitives に置換
- **同一機能の重複コンポーネント** → variant/size で統合

依存方向（primitives ← components ← patterns ← layouts）を徹底する。
**project/* と main の両方で使用可能。**

**処理フロー:**

1. **外部依存・プリミティブHTMLの検出**
   - patterns/ と layouts/ 内のファイルをスキャン
...
3. **対応方針の選択**
   ...
   - **B. 新規コンポーネント作成**: 不足している primitives/components を作成してから置換
   - **C. スキップ**: 外部依存を維持（複雑すぎる場合など）
   - **D. ラッパー作成**: 外部コンポーネントを primitives でラップして抽象化

4. **置換の実行**
   - インポートパスを `@ui-catalog/core/primitives` 等に変更
   - props のマッピングを調整

5. **新規コンポーネント作成（必要な場合）**
   - 不足している primitives/components を作成
   - VERSION_REGISTRY に登録

6. **依存方向の検証**
   - primitives ← components ← patterns ← layouts の方向のみ許可
   - 逆方向の依存があればエラー
...
`VERSION_REGISTRY`（ui-catalog 側）とアプリ側の `ui-catalog.versions.json` を同期する。
**project/* と main の両方で使用可能。**

**処理フロー:**

1. **差分検出**
   - `packages/ui-catalog/infra/version/registry.ts` の `VERSION_REGISTRY` を読み込み
   - アプリ側の `ui-catalog.versions.json` を読み込み
   - 差分を検出
...
2. **差分の一覧表示**
   ...
   【バージョン更新】
   ┌────────────────┬──────────┬──────────┐
   │ コンポーネント  │ versions │ REGISTRY │
   ├────────────────┼──────────┼──────────┤
   │ Button         │ 1.0.0    │ 1.1.0    │
   │ Icon           │ 1.3.0    │ 1.4.0    │
   └────────────────┴──────────┴──────────┘

   【新規コンポーネント（REGISTRY にあり、versions.json にない）】
   - MemberCard: 1.0.0
   - StatusBadge: 1.0.0

   【削除候補（versions.json にあり、REGISTRY にない）】
   - OldComponent
   ```

3. **同期方向の選択**
   - **A. REGISTRY → versions.json**: ui-catalog の最新を反映（通常はこちら）
   - **B. 差分確認のみ**: 変更せずに終了

4. **同期の実行**
   - `ui-catalog.versions.json` を更新
   - 削除候補があれば確認後に削除
...
5. **変更サマリーを報告**
   ...
   更新: 2件（Button, Icon）
   追加: 2件（MemberCard, StatusBadge）
   削除: 1件（OldComponent）

   ✅ ui-catalog.versions.json を更新しました
   ```
...
💡 推奨アクション:
1. project/pleasync から 3件をマージ → /ui-catalog mg
2. 全 project/* に 1件を配布 → /ui-catalog ds
```
