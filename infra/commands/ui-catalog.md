...
**間違ったブランチで実行したら即エラーで止める。例外なし。**

```
❌ main で /ui-catalog sync は実行できません。project/* ブランチに切り替えてください。
```
...
main の最新を現在の project/* ブランチに merge で取り込む。
...
2. merge 可能な project/* ブランチを一覧表示（独自コミットがあるもの）
3. ユーザーが対象ブランチを選択
4. `git fetch origin project/<name>`
5. `git merge origin/project/<name>`
6. コンフリクト発生時 → 解決方法を案内（自動解決しない）
