...
│  core/                 absorb（外部から吸収）        │
│    atoms/              refine（洗練・整理）          │
│    molecules/          消込（昇格後の残骸除去）       │
│    organisms/          distribute（各プロジェクトへ） │
│    templates/          merge（安定版へ統合）         │
│                                                     │
│  【観測機構】                                        │
│  infra/                                             │
│    devtools/（操作ログ）                             │
...
昇格完了後、extensions/ 側の残骸を除去し、エクスポートを整合させる。

**消込は昇格と同一 PR 内で行うこと。** 別 PR にすると重複状態が main にマージされる。

**消込チェックリスト:**
- [ ] `extensions/<project>/` から昇格元のディレクトリを削除
- [ ] `extensions/<project>/index.ts` からエクスポートを削除
- [ ] core/ 版が SCSS Module で実装されていることを確認
- [ ] VERSION_REGISTRY の登録を core/ セクションに移動（extension セクションから削除）
- [ ] リポジトリ全体で旧パスのインポートを検索・更新（`grep -r 'extensions/<project>/<Component>'`）
- [ ] `pnpm sync:versions` を実行
- [ ] `tsc --noEmit` で重複エクスポートがないことを確認
- [ ] PR 本文に移行パスを記載（旧インポート → 新インポート）

**下流アプリへの段階的移行が必要な場合:**

即時削除が難しいときは、extensions/ 側を **deprecation re-export** に置き換える:

```typescript
// extensions/1on1/index.ts
/** @deprecated @ui-catalog/core/molecules からインポートしてください */
export { StatusBar } from '../../core/molecules/StatusBar/StatusBar'
```

次リリースで re-export ごと削除する。`/ui-catalog clean` が deprecation re-export を検出・警告する。
