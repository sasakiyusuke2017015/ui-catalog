...
const TrendChart: FC<TrendChartProps> = ({
  ...
  // Y軸の範囲を計算(maxValueが指定されている場合はそれを使用)
  // Note: yDomainは現在固定値を使用しているため、動的計算はコメントアウト
  // const yDomain = maxValue !== null ? [minValue, maxValue] : ...
  ...
  useEffect(() => {
    ...
    data.datasets.forEach((dataset, datasetIndex) => {
      ...
      dataset.data.forEach((value, index) => {
        if (value === null) return; // nullの場合はスキップ
        ...
        if (index > 0) {
          ...
          if (prevValue === null) return; // 前の値がnullの場合は軌跡をスキップ
  ...
  const handleLineClick = (
    ...
    // normal ↔ bold のトグル（datasetStatesと同期）
    setDatasetStates((prev) => {
      const currentState = prev[datasetLabel] || 'normal';
      // hiddenの時はクリックできないが、念のためhiddenは維持
  ...
  const CustomTooltip = ({
    ...
    if (active && payload && payload.length) {
      return (
        <div className="border border-gray-200 bg-white p-3 shadow-lg" style={{ borderRadius: cardRadius }}>
          <p className="mb-2 text-fluid-sm font-medium text-gray-900">{label}</p>
  ...
  const CustomLegend = () => {
    return (
      <div className="mb-4 flex justify-center outline-none">
        <div className="inline-flex flex-wrap items-center gap-4 border border-gray-200 bg-white/90 px-4 py-2 shadow-md outline-none backdrop-blur-sm" style={{ borderRadius: cardRadius }}>
          {internalData.datasets.map((dataset, index) => {
            ...
            return (
              <div
                key={`item-${index}`}
                className="flex cursor-pointer items-center gap-2 outline-none transition-opacity hover:opacity-70"
      ...
      <g transform={`translate(${x},${y})${shouldRotateLabels ? ' rotate(-45)' : ''}`}>
        <text
          ...
          className={
            isCurrentMonth
              ? 'transition-colors'
              : 'transition-colors hover:fill-blue-600'
          }
        >
          {payload.value}
        </text>
