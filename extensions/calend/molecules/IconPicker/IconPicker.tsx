...
export function IconPicker({ value, onChange, size = 28 }: IconPickerProps) {
  return (
    <div data-component="IconPicker" role="radiogroup" aria-label="アイコン選択" className="flex gap-1 flex-wrap">
      {EVENT_ICONS.map((item) => {
        ...
        return (
          ...
          >
            {item.value ? (
              <Icon name={item.value as IconName} size={size * 0.6} className="transition-transform hover:scale-125" />
            ) : (
              <span className="transition-transform hover:scale-125" style={{ fontSize: size * 0.4, lineHeight: 1, color: '#9ca3af' }}>—</span>
            )}
          </button>
        )
      })}
