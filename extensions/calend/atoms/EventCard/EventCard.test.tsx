...
describe('EventCard', () => {
  describe('compact variant', () => {
    it('renders title', () => {
      render(<EventCard variant="compact" title="Lunch" color="#4f46e5" />)
    ...
    it('applies background color', () => {
      render(<EventCard variant="compact" title="Lunch" color="#dc2626" />)
      const el = screen.getByText('Lunch')
      expect(el.style.backgroundColor).toBe('rgb(220, 38, 38)')
    })

    it('calls onClick when clicked', () => {
      const onClick = vi.fn()
      render(<EventCard variant="compact" title="Lunch" color="#4f46e5" onClick={onClick} />)
    ...
    it('renders delete button when onDelete is provided', () => {
      ...
      render(<EventCard variant="compact" title="Lunch" color="#4f46e5" onDelete={onDelete} />)
    ...
    it('applies isDragging styles', () => {
      render(<EventCard variant="compact" title="Lunch" color="#4f46e5" isDragging />)
      const el = screen.getByText('Lunch')
    ...
    it('renders children', () => {
      render(
        <EventCard variant="compact" title="Lunch" color="#4f46e5">
  ...
  describe('timeline variant', () => {
    ...
    it('calls onClick when clicked', () => {
      const onClick = vi.fn()
      render(<EventCard {...baseProps} onClick={onClick} />)
    ...
    it('renders delete button when onDelete is provided', () => {
      const onDelete = vi.fn()
      render(<EventCard {...baseProps} onDelete={onDelete} />)
      const deleteBtn = screen.getByRole('button', { name: 'Delete event' })
      fireEvent.click(deleteBtn)
      expect(onDelete).toHaveBeenCalledTimes(1)
    })
    ...
    it('applies percent-based positioning when leftPercent and widthPercent are set', () => {
      render(<EventCard {...baseProps} leftPercent={25} widthPercent={50} />)
      const el = screen.getByText('Meeting').closest('[data-component]') as HTMLElement
